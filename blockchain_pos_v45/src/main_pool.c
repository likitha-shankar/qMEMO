...
static Transaction* pb_to_transaction(const Blockchain__Transaction* pt) {
    ...
    if (pt->signature.data && pt->signature.len > 0) {
        size_t sig_len = pt->signature.len < 48 ? pt->signature.len : 48;
...
int main(int argc, char* argv[]) {
    ...
    if (blockchain_pub_addr) {
        ...
        LOG_INFO("🔌 SUB → blockchain PUB at %s (for confirmations)", blockchain_pub_addr);
    } else {
        ...
        if (items[0].revents & ZMQ_POLLIN) {
            ...
            if (size > 0) {
                ...
                // ==========================================================
                    ...
                    LOG_INFO("📤 Fetching up to %u pending TXs for block #%u...", 
                             max_count, block_height);
                    
                    uint32_t count = 0;
                    Transaction** txs = pool_get_pending(pool, max_count, block_height, &count);
                    ...
                    LOG_INFO("📤 ✅ Returned %u TXs (fees: %lu, %zu bytes protobuf)", 
