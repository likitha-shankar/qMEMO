...
| Finding | Detail |
|---------|--------|
| **Signature verification was stubbed out** | `crypto_verify()` returned `true` without checking — baseline TPS numbers measured everything except real crypto |
...
| **Bug fixes** | `sig_type` missing in block.c/validator.c deserialize; `main_wallet.c` hardcoded SIG_ECDSA for batch_send |
