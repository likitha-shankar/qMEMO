...
//   value              = 8 bytes
//   fee                = 4 bytes
// Crypto fields (variable — supports Ed25519, Falcon-512, ML-DSA-44):
//   signature          = up to CRYPTO_SIG_MAX bytes
//   sig_len            = actual sig bytes used
//   public_key         = up to CRYPTO_PUBKEY_MAX bytes
//   pubkey_len         = actual pubkey bytes used
//   sig_type           = SIG_ED25519 / SIG_FALCON512 / SIG_ML_DSA44 / SIG_HYBRID
// =============================================================================
...
typedef struct {
    ...
    uint8_t signature[CRYPTO_SIG_MAX];           // up to 2420 bytes
    size_t  sig_len;                             // actual sig bytes
    uint8_t public_key[CRYPTO_PUBKEY_MAX];       // up to 1312 bytes
    size_t  pubkey_len;                          // actual pubkey bytes
    uint8_t sig_type;                            // which scheme
} Transaction;
...
// Hash covers only the core economic fields (not sig/pubkey)
...
// Verify using tx->sig_type + embedded public key — safe for OpenMP
