...
K_PARAM=${3:-16}
NUM_FARMERS=${4:-10}
WARMUP_BLOCKS=${5:-auto}  # "auto" calculates based on transactions needed
...
# Auto-calculate warmup blocks if "auto" or if user-provided value is too low
# Formula: need 2 coins per transaction, ~50 coins per block
calculate_warmup_blocks() {
    local requested_tx=$1
    local user_warmup=$2
    
    # Each transaction costs 2 coins (1 value + 1 fee)
    local coins_needed=$((requested_tx * 2))
    
    # Add 20% safety margin for fee collection variance
    local coins_with_margin=$((coins_needed * 120 / 100))
    
    # Calculate blocks needed (50 coins base reward per block)
    local blocks_needed=$((coins_with_margin / BASE_MINING_REWARD + 1))
    
    # Ensure at least 10 blocks for plot generation
    if [ $blocks_needed -lt 10 ]; then
        blocks_needed=10
    fi
    
    # If user provided a value, use the maximum of calculated and user value
    if [ "$user_warmup" != "auto" ]; then
        if [ $user_warmup -lt $blocks_needed ]; then
            echo "auto-scaling warmup: $user_warmup -> $blocks_needed blocks (need $coins_needed coins)" >&2
            echo $blocks_needed
        else
            echo $user_warmup
        fi
...
# Calculate warmup blocks
WARMUP_BLOCKS=$(calculate_warmup_blocks $NUM_TRANSACTIONS "$WARMUP_BLOCKS")
...
$BUILD_DIR/pool "tcp://*:$POOL_PORT" --sub "tcp://localhost:$BLOCKCHAIN_PUB_PORT" > "$BENCHMARK_DIR/pool.log" 2>&1 &
POOL_PID=$!
sleep 1
...
echo "  Waiting for validators to generate plots and mine blocks..."
echo ""
COINS_EXPECTED=$((WARMUP_BLOCKS * BASE_MINING_REWARD))
echo "  Expected coins: ~$COINS_EXPECTED (${WARMUP_BLOCKS} blocks × ${BASE_MINING_REWARD} coins/block)"
echo "  Coins needed: $((NUM_TRANSACTIONS * 2)) (${NUM_TRANSACTIONS} tx × 2 coins/tx)"
echo ""

# Calculate warmup time - give extra time for plot generation
PLOT_GEN_TIME=$((10 + K_PARAM / 4))  # More time for plot generation
WARMUP_TIME=$((PLOT_GEN_TIME + WARMUP_BLOCKS * BLOCK_INTERVAL + 10))

echo "  Phase 1: Plot generation (~${PLOT_GEN_TIME}s)..."
for i in $(seq 1 $PLOT_GEN_TIME); do
    printf "\r  Waiting for plots: %d / %d seconds" "$i" "$PLOT_GEN_TIME"
    sleep 1
done
echo ""
echo "  ✓ Plots should be ready"
echo ""
echo "  Phase 2: Mining warmup blocks (~$((WARMUP_BLOCKS * BLOCK_INTERVAL))s for ${WARMUP_BLOCKS} blocks)..."

MINING_WAIT=$((WARMUP_BLOCKS * BLOCK_INTERVAL + 5))
for i in $(seq 1 $MINING_WAIT); do
    # Check chain height periodically
    if (( i % 5 == 0 )); then
        CURRENT_HEIGHT=$(get_height)
        printf "\r  Mining progress: %d / %d seconds (chain height: %s)" "$i" "$MINING_WAIT" "$CURRENT_HEIGHT"
    else
        printf "\r  Mining progress: %d / %d seconds" "$i" "$MINING_WAIT"
    fi
    sleep 1
done
echo ""
...
# Check metronome
if ! kill -0 $METRONOME_PID 2>/dev/null; then
    echo -e "${RED}Metronome died during warmup${NC}"
    tail -20 "$BENCHMARK_DIR/metronome.log"
    exit 1
fi

print_header "VERIFYING INITIAL STATE"

# Wait for at least one farmer to have balance (with retries)
echo "  Checking farmer balances (will retry if needed)..."
MAX_BALANCE_RETRIES=10
RETRY_COUNT=0

while [ $RETRY_COUNT -lt $MAX_BALANCE_RETRIES ]; do
    # Calculate total available coins from all farmers
    TOTAL_AVAILABLE_COINS=0
    declare -A FARMER_BALANCES
    declare -A FARMER_NONCES
    
    for i in $(seq 1 $NUM_FARMERS); do
        FARMER_BALANCES[$i]=$(get_balance "farmer$i")
        FARMER_NONCES[$i]=$(get_nonce "farmer$i")
        TOTAL_AVAILABLE_COINS=$((TOTAL_AVAILABLE_COINS + ${FARMER_BALANCES[$i]}))
    done
    
    if [ "$TOTAL_AVAILABLE_COINS" -gt 0 ]; then
        echo ""
        echo "  ✓ Farmers have coins!"
        break
    fi
    
    RETRY_COUNT=$((RETRY_COUNT + 1))
    printf "\r  Retry %d/%d: No coins yet, waiting for mining..." "$RETRY_COUNT" "$MAX_BALANCE_RETRIES"
    sleep 3
done
echo ""
...
for i in $(seq 1 $NUM_FARMERS); do
    FARMER_BALANCES[$i]=$(get_balance "farmer$i")
    FARMER_NONCES[$i]=$(get_nonce "farmer$i")
    echo "  Farmer$i: ${FARMER_BALANCES[$i]} coins, nonce ${FARMER_NONCES[$i]}"
    TOTAL_AVAILABLE_COINS=$((TOTAL_AVAILABLE_COINS + ${FARMER_BALANCES[$i]}))
done
...
# If still 0 coins, show diagnostic info
if [ "$TOTAL_AVAILABLE_COINS" -eq 0 ]; then
    echo ""
    echo -e "${RED}⚠️  WARNING: Farmers have 0 coins after warmup!${NC}"
    echo "  Checking logs..."
    echo ""
    echo "  Last metronome log entries:"
    tail -10 "$BENCHMARK_DIR/metronome.log" 2>/dev/null | head -5
    echo ""
    echo "  Last farmer1 log entries:"
    tail -10 "$BENCHMARK_DIR/farmer1.log" 2>/dev/null | head -5
    echo ""
    echo "  This might indicate validators aren't winning blocks."
    echo "  Consider: increase warmup time, reduce difficulty, or check connectivity"
fi

# Calculate max transactions (each tx costs 2 coins: 1 value + 1 fee)
MAX_POSSIBLE_TX=$((TOTAL_AVAILABLE_COINS / 2))
if [ "$NUM_TRANSACTIONS" -gt "$MAX_POSSIBLE_TX" ]; then
    echo ""
    echo -e "${YELLOW}⚠️  Requested $NUM_TRANSACTIONS tx but only $MAX_POSSIBLE_TX possible${NC}"
    echo "  (Need $((NUM_TRANSACTIONS * 2)) coins but only have $TOTAL_AVAILABLE_COINS)"
    echo "  Tip: Increase warmup blocks or reduce transaction count"
    NUM_TRANSACTIONS=$MAX_POSSIBLE_TX
    echo "  Adjusted to $NUM_TRANSACTIONS transactions"
fi

print_header "RUNNING END-TO-END TRANSACTION BENCHMARK (2-PHASE)"

# Calculate transactions per farmer
TX_PER_FARMER=$((NUM_TRANSACTIONS / NUM_FARMERS))
REMAINING_TX=$((NUM_TRANSACTIONS % NUM_FARMERS))

echo ""
echo "Distribution:"
print_metric "Transactions per farmer" "$TX_PER_FARMER"
print_metric "Extra transactions for farmer1" "$REMAINING_TX"
echo ""

# Record start time and height
HEIGHT_BEFORE=$(get_height)

# 2-PHASE BENCHMARK: Submit all TXs first, THEN process blocks
#
# Phase A: PAUSE metronome → submit ALL transactions → measure submission TPS
# Phase B: RESUME metronome → wait for all confirmations → measure processing TPS

BLOCK_TIMES_FILE="$BENCHMARK_DIR/block_times.tmp"
# Scale MAX_WAIT based on transaction count (at ~5K TPS processing)
MAX_WAIT=$((NUM_TRANSACTIONS / 3000 + 60))
...
# PAUSE metronome (stop creating blocks during submission)
echo "  ⏸️  Pausing metronome for clean submission measurement..."
PAUSE_RESP=$(timeout 5 python3 -c "
import zmq
ctx = zmq.Context()
...
echo "  Metronome response: $PAUSE_RESP"
sleep 0.5
...
echo ""
echo "═══════════════════════════════════════════════════════════════════════════"
echo "  PHASE A: SUBMISSION (metronome paused, no blocks being created)"
echo "═══════════════════════════════════════════════════════════════════════════"
echo ""
echo "Submitting $NUM_TRANSACTIONS transactions from $NUM_FARMERS farmers..."
echo "Using $NUM_THREADS OpenMP threads per farmer × $NUM_FARMERS parallel farmers (batch=$BATCH_SIZE)"
echo "Nonce ordering: Pool accepts any order → sorts by nonce at block creation"
echo ""

# Store submit times for latency tracking
declare -A TX_SUBMIT_TIMES

# Launch all farmers in PARALLEL for maximum submission throughput
echo "  Launching $NUM_FARMERS farmers in parallel..."
FARMER_PIDS=()
FARMER_RESULTS=()
for i in $(seq 1 $NUM_FARMERS); do
    TX_COUNT=$TX_PER_FARMER
    if [ "$i" -eq 1 ]; then
        TX_COUNT=$((TX_PER_FARMER + REMAINING_TX))
    fi
...
# Wait for all farmers to complete and collect results
echo "  Waiting for all farmers to complete..."
for i in $(seq 1 $NUM_FARMERS); do
    if [ -n "${FARMER_PIDS[$i]}" ]; then
        wait ${FARMER_PIDS[$i]} 2>/dev/null
        ...
        SUBMITTED=0
        if [ -f "${FARMER_RESULTS[$i]}" ]; then
            SUBMITTED=$(grep -oP 'BATCH_RESULT:\K\d+' "${FARMER_RESULTS[$i]}" | tail -1)
            if [ -z "$SUBMITTED" ]; then SUBMITTED=0; fi
        fi
        
        if [ "$SUBMITTED" -gt 0 ]; then
            echo "  Farmer$i: submitted $SUBMITTED/$TX_COUNT transactions"
            TX_SUBMITTED=$((TX_SUBMITTED + SUBMITTED))
        else
            echo -e "  Farmer$i: ${RED}ERRORS${NC} ($TX_COUNT requested)"
...
for i in $(seq 1 $NUM_FARMERS); do
    if [ -f "$SUBMIT_PROGRESS_DIR/farmer${i}.progress" ]; then
        grep "SUBMIT_PROGRESS:" "$SUBMIT_PROGRESS_DIR/farmer${i}.progress" 2>/dev/null | \
            sed 's/SUBMIT_PROGRESS://' | \
            awk -F: '{printf "%.3f,%s,farmer'$i'\n", $1/1000.0, $2}' >> "$SUBMIT_PROGRESS_CSV"
    fi
done
...
echo ""
echo "═══════════════════════════════════════════════════════════════════════════"
echo "  PHASE B: BLOCK PROCESSING (resuming metronome)"
echo "═══════════════════════════════════════════════════════════════════════════"
echo ""

# Start PUB/SUB block monitor BEFORE resuming
$BUILD_DIR/wallet wait_confirm bench_receiver "$TX_SUBMITTED" "$RECEIVER_INITIAL" \
    --timeout "$MAX_WAIT" \
    --pub-addr "tcp://localhost:$BLOCKCHAIN_PUB_PORT" \
    2>"$BENCHMARK_DIR/confirm_progress.log" \
    >"$BLOCK_TIMES_FILE" &
CONFIRM_PID=$!
sleep 0.3  # Let SUB socket connect
echo "  ✓ Block monitor started (PID: $CONFIRM_PID)"

# RESUME metronome
PROCESS_START=$(date +%s.%N)
PROCESS_START_MS=$(date +%s%3N)
echo "  ▶️  Resuming metronome..."
RESUME_RESP=$(timeout 5 python3 -c "
import zmq
ctx = zmq.Context()
s = ctx.socket(zmq.REQ)
s.setsockopt(zmq.RCVTIMEO, 3000)
s.connect('tcp://localhost:$METRONOME_REP_PORT')
s.send_string('RESUME')
print(s.recv_string())
s.close(); ctx.term()
" 2>/dev/null || echo "FAILED")
echo "  Metronome response: $RESUME_RESP"

if [ "$TX_SUBMITTED" -lt "$NUM_TRANSACTIONS" ]; then
    echo ""
    echo "  ⚠️  Only $TX_SUBMITTED of $NUM_TRANSACTIONS submitted (some rejected by pool)"
fi
...
# IMMEDIATELY pause metronome to stop creating empty/coinbase-only blocks
# while we gather metrics, generate graphs, etc.
timeout 5 python3 -c "
import zmq
ctx = zmq.Context()
s = ctx.socket(zmq.REQ)
...
s.close(); ctx.term()
" >/dev/null 2>&1
echo "  ✓ Metronome paused (all confirmations received)"
...
# Verify confirmed count with direct balance query
RECEIVER_FINAL=$(get_balance bench_receiver)
TX_CONFIRMED=$((RECEIVER_FINAL - RECEIVER_INITIAL))
...
VALID_PROOFS=$(grep -c "VALID PROOF" "$BENCHMARK_DIR/metronome.log" 2>/dev/null || echo "0")
PROOFS_FOUND=0
for i in $(seq 1 $NUM_FARMERS); do
...
# Calculate final balances and track mining rewards
TOTAL_SPENT=0
TOTAL_MINING_REWARDS=0
for i in $(seq 1 $NUM_FARMERS); do
    FINAL_BAL=$(get_balance "farmer$i")
    SPENT=$((${FARMER_BALANCES[$i]} - FINAL_BAL))
    TOTAL_SPENT=$((TOTAL_SPENT + SPENT))
    
    # If farmer gained money (SPENT < 0), they received mining rewards
    if [ $SPENT -lt 0 ]; then
        MINING_GAIN=$((-SPENT))
    else
        MINING_GAIN=0
    ...
    print_metric "Farmer$i (initial → final)" "${FARMER_BALANCES[$i]} → $FINAL_BAL (Δ: $((FINAL_BAL - ${FARMER_BALANCES[$i]})))"
done
