...
bool validator_create_and_submit_block(Validator* v) {
    ...
    uint8_t* tx_pubkeys = safe_malloc(max_txs_per_block * 32);
    memset(tx_pubkeys, 0, max_txs_per_block * 32);
    uint32_t tx_count = 0;
        ...
        if (batch) {
            for (size_t i = 0; i < batch->n_transactions && tx_count < max_txs_per_block; i++) {
                ...
                if (pt->public_key.data && pt->public_key.len >= 32)
                    memcpy(tx_pubkeys + tx_count * 32, pt->public_key.data, 32);
                txs[tx_count++] = tx;
            }
    ...
    if (!block) {
        ...
        free(buffer); free(txs); free(tx_pubkeys);
        return false;
    }
    ...
    for (uint32_t batch_start = 0; batch_start < tx_count && !block_full; batch_start += VERIFY_BATCH_SIZE) {
        ...
        //   1. Check pubkey is available (stored by pool alongside TX)
        //   2. Verify hash160(pubkey) == source_address (pubkey ownership)
        //   3. Recompute tx_hash = BLAKE3(nonce||expiry||src||dst||value||fee)
        //   4. Ed25519_verify(pubkey, tx_hash, signature)
        //
        // With 8 OpenMP threads and 1000 TXs per batch:
        //   125 TXs/thread × ~0.06ms/verify = ~7.5ms per batch
        // ──────────────────────────────────────────────────────────────
        ...
        for (uint32_t bi = 0; bi < batch_size; bi++) {
            ...
            // Get the Ed25519 pubkey stored by pool alongside this TX
            const uint8_t* pubkey = tx_pubkeys + i * 32;
            
            if (!is_zero(pubkey, 32)) {
                // Full Ed25519 verification:
                // 1. hash160(pubkey) must match source_address
                // 2. Ed25519_verify(pubkey, BLAKE3(tx_fields), signature)
                if (!transaction_verify_ed25519(tx, pubkey)) {
                    batch_valid[bi] = 0;
                    batch_sig_fail++;
                }
            } else {
                // No pubkey: fall back to basic sanity check (non-zero sig)
