...
typedef struct Wallet {
    ...
    uint8_t address[20];         // 20-byte binary address = hash160(ed25519_pubkey)
    ...
    uint8_t public_key[65];      // Legacy: Uncompressed EC public key
    size_t public_key_len;       // Legacy: Actual public key length
    EVP_PKEY* evp_key;           // OpenSSL key handle (Ed25519 or EC)
    char private_key_pem[2048];  // PEM-encoded private key (legacy EC)
...
// Sign message with wallet's Ed25519 private key (returns 64-byte signature)
bool wallet_sign(const Wallet* wallet, const uint8_t* message, size_t msg_len,
                 uint8_t signature[64]);

// Get wallet's Ed25519 public key (32 bytes)
...
void txhash_to_hex(const uint8_t hash[32], char hex[65]);
